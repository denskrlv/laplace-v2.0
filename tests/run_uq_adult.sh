...
python tests/uq.py ${COMMON_ARGS} --method map --run_name "${BENCHMARK}/map_baseline"

echo ""
echo "--> RUNNING BASELINE LAPLACE (LAST-LAYER) EXPERIMENT"
# This corresponds to the default "LA" in the paper
python tests/uq.py ${COMMON_ARGS} --method laplace --subset_of_weights last_layer --run_name "${BENCHMARK}/laplace_ll_baseline"
...
python tests/uq.py ${COMMON_ARGS} --method laplace --subset_of_weights last_layer --domain_shift_gender male_to_female --run_name "${BENCHMARK}/shift_male_to_female"

echo ""
echo "--> RUNNING DOMAIN SHIFT: FEMALE-TO-MALE"
python tests/uq.py ${COMMON_ARGS} --method laplace --subset_of_weights last_layer --domain_shift_gender female_to_male --run_name "${BENCHMARK}/shift_female_to_male"
...
for intensity in 0.1 0.25 0.5 0.75 1.0; do
    ...
    python tests/uq.py ${COMMON_ARGS} --method laplace --subset_of_weights last_layer --noise_intensity ${intensity} --run_name "${BENCHMARK}/noise_${intensity}"
done
